...
class Section(BaseModel):
    ...
    type: SectionType #Esse type recebe do SectionType 

class PageResponse(BaseModel):
    model_config = ConfigDict(arbitrary_types_allowed=True)
    page_id: int
    title_page: Optional[str] = None 
    status: PageStatus = Field(..., json_schema_extra={"enum": [s.value for s in PageStatus]})
    sections: List[Section] = []
    #Acrescentar snapshot posteriormente
